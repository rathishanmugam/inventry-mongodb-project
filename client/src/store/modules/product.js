  ...
  saveProduct({commit, dispatch, state, rootState}, product) {
    ...
    if (!oldProduct) {
      Vue.axios.post('/product', product)
        .then((resp) => {
          commit('addProductToState', product).then(() => {
    ...
    if (cartItem) {
      ...
      Vue.axios.put('/product/' + this.state.product.productId, product)
        .then((resp) => {
          ...
          commit('updateProductInState', product).then(() => {
            dispatch('items') //in product.vue component
          })
        })
        .catch((err) => {
    ...
    if (cartItem) {
      ...
      Vue.axios.delete('/product/' + cartItem.productId, product)
        .then((resp) => {
          ...
          commit('deleteProductInState', product).then(() => {
            dispatch('items') //in product.vue component
          })
        })
        .catch((err) => {
    ...
    Vue.axios.get('/product')
      .then((resp) => {
        ...
        if (data && data.length > 0) {
          commit('displayProductToState', data).then(() => {
            dispatch('items') //in product.vue component
          })
        }
      })
